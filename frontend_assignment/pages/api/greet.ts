...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const errorMessage = error.reason || JSON.parse(error.body).error.message
        const responseMessage = errorMessage.substring(errorMessage.indexOf("'") + 1, errorMessage.lastIndexOf("'"))
        res.status(500).send(responseMessage || "Unknown error!")
    }
}
